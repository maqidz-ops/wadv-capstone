...
const authService = {
  ...
  async refresh(tokenString) {
    ...
    try {
      ...
      const err2 = new Error('Refresh token tidak valid atau sudah expired.');
      err2.statusCode = 401; err2.code = 'INVALID_REFRESH_TOKEN';
      throw err2;
    }
    ...
    if (storedToken && storedToken.isRevoked) {
      ...
      const err3 = new Error('Token reuse detected. Silakan login ulang.');
      err3.statusCode = 401; err3.code = 'TOKEN_REUSE_DETECTED';
      throw err3;
    }

    // 4. Token tidak ditemukan di DB (mungkin sudah di-delete atau tidak valid)
    if (!storedToken) {
      const err4 = new Error('Refresh token tidak ditemukan.');
      err4.statusCode = 401; err4.code = 'INVALID_REFRESH_TOKEN';
      throw err4;
    }
    ...
    // 6. Buat token baru
    const user = await userRepo.findById(payload.sub || payload.userId || storedToken.userId);
    if (!user) {
      const err5 = new Error('User tidak ditemukan.');
      err5.statusCode = 404; err5.code = 'USER_NOT_FOUND';
      throw err5;
    }
